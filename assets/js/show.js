        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (parse.CountSoal > 0) {
                    ...
                    // pesan_warning('Pesan', 'sedang rekapitulasi nilai :)');
                } else {
...
function showPoskoProduk(data, count, tipe = 'posko') {
    ...
    let TampungNilai;
    let EditTotal = [], EditTotal2 = [];
    let nilai1 = 0, nilai2 = 0;
    // var date = 
    for (let i = 0; i < count; i++) {
        ...
        html += "<th width=10% class='text-center'>NILAI 2</th>";
        ...
        for (let y = 0; y < source[i].soal.length; y++) {
            ...
            nilai1 = null; nilai2 = null;

            if (typeof Edit[source[i].id_ceklis] != 'undefined') {
                TampungNilai = Edit[source[i].id_ceklis][source[i].soal[y].id_soal_kelompok];

                Total += parseFloat(TampungNilai['nilai']);
                Total2 += parseFloat(TampungNilai['nilai2']);

                nilai1 = TampungNilai['nilai'];
                nilai2 = TampungNilai['nilai2'];
            ...
            html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                "' value='" + (nilai1 ?? '') + "' name='nilai[" + source[i].id_ceklis + "][" + (source[i].soal[y].id_soal_kelompok) + "]' class='form-control nilai_" + (i) + "' placeholder='Nilai' onchange='TotalNilai(this, " + (i) + ", " + (i) + ")'></td>";

            html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                "' value='" + (nilai2 ?? '') + "' name='nilai_2[" + source[i].id_ceklis + "][" + (source[i].soal[y].id_soal_kelompok) + "]' class='form-control nilai2_" + (i) + "' placeholder='Nilai' onchange='TotalNilai2(this, " + (i) + ", " + (i) + ")'></td>";
        ...
        html += "<th class='text-center bg-danger text-light montserrat-600 total_nilai2'>" + (Total2 != 0 ?
            Total2.toFixed(2) : 0) + "</th>";
...
function TotalNilai(that, index, indexceklis, countjabatan, jmlanggota = 1) {
    ...
    if (nilai > max || nilai < 0) {
        ...
        const TotalNilai2 = parseFloat($('.total_nilai2').eq(index).text()) ?? 0;
        $('.total-nilai-accordion').eq(indexceklis).text(parseFloat((TampungNilai + TotalNilai2) / 2).toFixed(2));
...
function TotalNilai2(that, index, indexceklis, countjabatan, jmlanggota = 1) {
    const nilai = parseFloat(that.value);
    const max = parseFloat(that.max);
    const Class = $(that)[0].classList[1];
    const data = $('.' + Class);
    ...
    if (nilai > max || nilai < 0) {
        ...
        $('.total_nilai2').eq(index).text(TampungNilai.toFixed(2))

        const TotalNilai1 = parseFloat($('.total_nilai').eq(index).text()) ?? 0;

        $('.total-nilai-accordion').eq(indexceklis).text(parseFloat((TampungNilai + TotalNilai1) / 2).toFixed(2))
    }
    // alert("TOTNIL : " + indexroot);
...
function TotalNilaiAsisten(that, indexratarata, indexasisten, indexcekklis, indextotal, countjabatan, jmlanggota = 1, jmlsoal = 1) {
    // alert(jmlsoal);
    const nilai = parseFloat(that.value);
    const max = parseFloat(that.max);
    const Class = $(that)[0].classList[1];
    ...
    if (nilai > max || nilai < 0) {
        ...
        $('.total_nilai').eq(indextotal).html((TampungNilai / jmlsoal).toFixed(2));
    ...
    if (countjabatan == 2) {
        ...
        if (CekNilai(indexasisten) == true && CekNilai(indexasisten + 1) == true)
            RataRata = (parseFloat($('.total_nilai').eq(indexasisten).text()) + parseFloat($('.total_nilai').eq(indexasisten + 1).text())) / jmlsoal;
        ...
        RataRata = parseFloat($('.total_nilai').eq(indexasisten).text()) / countjabatan;
    }
...
function showProdukAktivitas(data, count) {
    ...
    let DataJabatan = [];
    let CEKLIS = 1;
    let Edit = [], z = 0, EditTotal1 = 0, EditTotal2 = 0, EditTotal3 = 0, EditTotal4 = 0, EditTotal5 = 0, EditAccordion = [];
    let i = 0, b = 0;
    let nilaioren = [];
    for (i = 0; i < count; i++) {
        EditTotal1 = 0;
        ...
        html += "<a style='margin:0px; padding:0px;' href='#' onclick='event.preventDefault()' class='btn m-0 btn-link coleps on' data-toggle='collapse' data-target='#collapse" + i + "' aria-expanded='true'>";
        html += "<div class='card-header col-lg-12 text-light bg-dark text-center d-flex justify-content-between montserrat-600 letter-spacing text-size-7' id='headingOne' data-target='#collapse" + i + "' style='margin:0px;'> <span class='text-size-7'><i class='fas fa-chevron-up icon-down text-size-4'></i> "
        html += CEKLIS++ + ' - ' + source[i].nama_ceklis + "</span> <span class='" + (source[i]['is_asisten'] == 1 || source[i]['is_asisten'] == 3 ? 'on_math' : '') + " total-nilai-accordion'>" + (sourceEdit[source[i].id_ceklis]['total_nilai'] ?? 0) + "</span></div></a>";
        ...
        if (source[i]['is_asisten'] == 1) {
            if (DataJabatan.length >= 1) {
                html += "<th width=10% class='text-center'>NILAI ASISTEN</th>";
            }
        ...
        for (let y = 0; y < source[i].soal.length; y++) {
            ...
            if (source[i]['is_asisten'] == 1 || source[i]['is_asisten'] == 3) {
                if (source[i]['is_asisten'] == 3) {
                    ...
                    if (DataJabatan.length == 1) {
                        // alert(DataJabatan.length);
                        html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                            "' name='nilai_1[" + source[i].id_ceklis + "][" + source[i].soal[y].id_soal_perorangan + "]' class='form-control nilai_" + (total_nilai_index) +
                            "' value='" + (Edit.length <= 0 ? '' : Edit[0][y]['nilai']) + "' placeholder='Nilai'  onchange='TotalNilaiAsisten(this," + (indexratarata) + "," + total_nilai_index + "," + (i) + "," + (total_nilai_index) + "," + (DataJabatan.length) + "," + (CountAnggota - 1) + "," + source[i].soal.length + ")' ></td> ";
                        ...
                        html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                            ...
                            "' value='" + (Edit.length <= 0 ? '' : Edit[1][y + source[i].soal.length]['nilai']) + "' placeholder='Nilai'  onchange='TotalNilaiAsisten(this," + (indexratarata) + "," + total_nilai_index + "," + (i) + "," + (total_nilai_index + 1) + "," + (DataJabatan.length) + "," + (CountAnggota - 1) + "," + source[i].soal.length + ")' ></td> ";
                        ...
                        html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                            ...
                            "' value='" + (Edit.length <= 0 ? '' : Edit[2][y + (source[i].soal.length * 2)]['nilai']) + "' placeholder='Nilai'  onchange='TotalNilaiAsisten(this," + (indexratarata) + "," + total_nilai_index + "," + (i) + "," + (total_nilai_index + 2) + "," + (DataJabatan.length) + "," + (CountAnggota - 1) + "," + source[i].soal.length + ")' ></td> ";
                        ...
                        html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                            ...
                            "' value='" + (Edit.length <= 0 ? '' : Edit[3][y + (source[i].soal.length * 3)]['nilai']) + "' placeholder='Nilai'  onchange='TotalNilaiAsisten(this," + (indexratarata) + "," + total_nilai_index + "," + (i) + "," + (total_nilai_index + 3) + "," + (DataJabatan.length) + "," + (CountAnggota - 1) + "," + source[i].soal.length + ")' ></td> ";
                        ...
                        html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                            ...
                            "' value='" + (Edit.length <= 0 ? '' : Edit[3][y + (source[i].soal.length * 3)]['nilai']) + "' placeholder='Nilai'  onchange='TotalNilaiAsisten(this," + (indexratarata) + "," + total_nilai_index + "," + (i) + "," + (total_nilai_index + 3) + "," + (DataJabatan.length) + "," + (CountAnggota - 1) + "," + source[i].soal.length + ")' ></td> ";
                        html += "<td><input type='number' step='0.01' max='" + source[i].soal[y].maks +
                            "' name='nilai_5[" + source[i].id_ceklis + "][" + source[i].soal[y].id_soal_perorangan + "]' class='form-control nilai_" + (total_nilai_index + 4) +
                            "' value='" + (Edit.length <= 0 ? '' : Edit[4][y + (source[i].soal.length * 4)]['nilai']) + "' placeholder='Nilai'  onchange='TotalNilaiAsisten(this," + (indexratarata) + "," + total_nilai_index + "," + (i) + "," + (total_nilai_index + 4) + "," + (DataJabatan.length) + "," + (CountAnggota - 1) + "," + source[i].soal.length + ")' ></td> ";
                    }
                }
                // console.log(Edit);
                if (Edit.length > 0) {
                    ...
                    if (source[i]['is_asisten'] == 1)
                        if (DataJabatan.length == 1)
                            EditTotal1 += EditTotal1 += parseFloat(Edit[0][y]['nilai']);
                    if (DataJabatan.length == 2)
                    ...
                    if (DataJabatan.length == 5)
                        EditTotal5 += parseFloat(Edit[4][y + (source[i].soal.length * 4)]['nilai']);
                }
        ...
        if (source[i]['is_asisten'] == 1) {
            ...
            if (EditTotal1 == 0)
                EditRata = (EditTotal1);
            if (EditTotal2 == 0)
                EditRata = (EditTotal1 + EditTotal2);
            if (EditTotal3 == 0)
                EditRata = (EditTotal1 + EditTotal2 + EditTotal3);
            if (EditTotal4 == 0)
                EditRata = (EditTotal1 + EditTotal2 + EditTotal3 + EditTotal4);
            if (EditTotal5 == 0)
            ...
            if (EditTotal1 > 0 && EditTotal2 > 0 && EditTotal3 > 0 && EditTotal4 > 0 && EditTotal5 > 0)
                EditRata = (EditTotal1 + EditTotal2 + EditTotal3 + EditTotal4 + EditTotal5) / 5;

            html += "<tr>";
            html += "<td colspan='3'>RATA RATA</td>";
            html += "<td colspan='5' class='bg-danger text-light text-center montserrat-600 rata-rata'>" + (sourceEdit[source[i].id_ceklis]['total_nilai'] ?? 0);
            html += "</td></tr>";
            // accordion
    ...
    for (c = 0; c < nilaioren.length; c++) {
        ...
        // console.log('Nilai Oren ' + totaloren);
    }
    // totaloren = 80;
    html += "<div class='card' style='border-radius:0px; border:0px solid transparent; margin:0px;'>";
    ...
    // html += "TOTAL NILAI AKTIVITAS HARI </span> <span id='nilai-oren' >" + (totaloren == 0 ? 0 : (totaloren / CountAnggota).toFixed(2)) + "</span></div></a>";
    // BUTTON 
    // html += buttonSave();
    $('#show').html(html);
